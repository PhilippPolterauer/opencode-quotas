import { renderQuotaBarParts, colorize } from "./progress-bar";
import { validateQuotaData } from "../utils/validation";
const DEFAULT_COLUMNS = ["status", "name", "percent", "bar", "reset", "ettl"];
const HEADERS = {
    name: "QUOTA NAME",
    bar: "UTILIZATION",
    percent: "USED",
...
export function renderQuotaTable(quotas, options) {
    if (quotas.length === 0)
        return [];
